 ...
 * they can be registered on driver failures.
  * 计算每秒从网络收到的用UTF8编码的文本中的字数,这个例子还展示了如何为累加器和广播使用延迟实例化的单例实例,
  * 以便它们可以在驱动程序失败时注册
 *
...
object RecoverableNetworkWordCount {
  ...
  def createContext(ip: String, port: Int, outputPath: String, checkpointDirectory: String)
    ...
    wordCounts.foreachRDD { (rdd: RDD[(String, Int)], time: Time) =>
      // Get or register the blacklist Broadcast 获取或注册黑名单广播
      val blacklist = WordBlacklist.getInstance(rdd.sparkContext)
      // Get or register the droppedWordsCounter Accumulator 获取或注册droppedWordsCounter累加器
      val droppedWordsCounter = DroppedWordsCounter.getInstance(rdd.sparkContext)
      // Use blacklist to drop words and use droppedWordsCounter to count them
      //使用黑名单放下单词,并使用droppedWordsCounter对它们进行计数
