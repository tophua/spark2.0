...
//类型聚合：expr
 ...
 //类型：表达式，表达式expr聚集
  ...
  //Kryo编码,将DataFrame转换为数据集时检查模式不匹配
  ...
  //使用ObjectProducer估算运算符中的sizeInBytes不应该失败
  ...
  //EmbedDeserializerInFilter不应该优化改变模式的计划片段
  test(
  ...
  //如果顶级产品输入对象为空,则数据集应抛出RuntimeException
  ...
  //修复KeyValueGroupedDataset中的序列化问题
  test("SPARK-18189: Fix serialization issue in KeyValueGroupedDataset") {
    ...
    //在KeyValueGroupedDataset中使用广播触发器序列化问题
  ...
  Seq(true, false).foreach { eager =>
      ...
      test(s"Dataset.checkpoint() - $testName (eager = $eager)") {
        withTempDir { dir =>
          ...
          try {
            ...
            //由于原来的checkpointDir可以是None,我们需要直接设置变量。
    ...
    testCheckpointing("basic") {
      ...
      //对于懒惰的checkpoint()调用,第一次检查也将实现检查点
  ...
  test ("SPARK-17460: the sizeInBytes in Statistics shouldn't overflow to a negative number") {
    ...
    //由于统计中的sizeInBytes可能超过Int的限制,我们应该使用BigInt而不是Int来避免可能的溢出。
    ...
    //sizeInBytes是2404280404,在修复之前,它溢出到一个负数
  ...
  //当启用ESCAPED_STRING_LITERALS时,不要转义正则表达式模式
 ...
 * Java serialization -- so the only way it can be "serialized" is through our encoders.
  *用于使用编码器测试序列化的类,这个类在使用Java序列化时抛出异常,
  * 所以唯一可以“串行化”的方式是通过我们的编码器
 */
