...
class PlannerSuite extends SharedSQLContext {
  ...
  //解释应该打印PRDYFilters of PhysicalRDD
  ...
  //高效的终端限制 - >排序应该使用TakeOrderedAndProject
  ...
  //终端限制 - >项目 - >排序应该使用TakeOrderedAndProject
  ...
  //不被TakeOrderedAndProject处理的终端限制应使用CollectLimit
  ...
  //akeOrderedAndProject可以出现在计划的中间
  ...
  ///////////////////////////////////////////////////////////////////////////
  // Unit tests of EnsureRequirements for Exchange 保证Exchange要求的单元测试
  ///////////////////////////////////////////////////////////////////////////
  ...
  test("EnsureRequirements with child partitionings with different numbers of output partitions") {
    ...
    //这与之前的测试类似,只是它检查分区不兼容,除非它们产生相同数量的分区
  ...
  test("EnsureRequirements with compatible child partitionings that do not satisfy distribution") {
    ...
    //左侧和右侧的输入具有可兼容的分区,但由于它们聚集在不同的列上,因此不能满足分配。因此,我们需要洗牌
  ...
  //如果孩子有相同的分区交换，EnsureRequirements消除交换
  ...
  ///////////////////////////////////////////////////////////////////////////
  // Unit tests of EnsureRequirements for Sort 保证排序要求的单元测试
  ///////////////////////////////////////////////////////////////////////////
  ...
  //内部SMJ之后需要连接键的任一侧时,EnsureRequirements将跳过排序
  ...
  test("EnsureRequirements skips sort when key order of a parent SMJ is propagated from its " +
    ...
    Seq(Inner, Cross).foreach { joinType =>
      ...
      //在第二个SMJ之后,exprA,exprB和exprC都应该被排序
  ...
  //EnsureRequirements在全外排序合并连接之后添加排序
  ...
  //EnsureRequirements在没有现有订单的情况下添加排序
  ...
  //如果需要排序是现有排序的前缀,EnsureRequirements将跳过排序
  ...
  // This is a regression test for SPARK-11135 这是SPARK-11135的回归测试
  //EnsureRequirements在需求排序不是现有排序的前缀时添加排序
