...
class DataFrameSuite extends QueryTest with SharedSQLContext {
  ...
  test("Star Expansion - CreateStruct and CreateArray") {
    ...
    //在aggregateExpressions中创建CreateStruct和CreateArray
    ...
    //项目列表中的CreateStruct和CreateArray（未解析的别名）
    ...
    //项目列表中的CreateStruct和CreateArray（别名）
  ...
  //将Scala Symbol的attrname转换为无法解析的属性
  ...
  //除了不同的SQL符合性,返回在当前集合存在的在其他集合不存在的
  ...
  //返回一个dataframe，在2个dataframe都存在的元素 - 可空性
  ...
  //drop（name：String）搜索并删除与名称匹配的所有顶级列
  ...
  test("SPARK-8072: Better Exception for Duplicate Columns") {
    // only one duplicate column present 只有一个重复的列存在
    val e = intercept[org.apache.spark.sql.AnalysisException] {
    ...
    // multiple duplicate columns present 存在多个重复列
  ...
  //使用随机列调用本地DataFrame上的`show`应返回相同的值
  ...
  //别名使用内部生成的名称“aggOrder”和“havingCondition”
  test("Alias uses internally generated names 'aggOrder' and 'havingCondition'") {
    ...
    //在计划中，有两个具有相同名称的属性“havingCondition”,一个是用户提供的别名,另一个是内部生成的
  ...
  //项目不应该通过相交或除外,返回一个dataframe，在2个dataframe都存在的元素
  ...
  //使用ClassCastException计算小数列的百分位数失败
  ...
  //由于JVM代码大小的限制,过滤器可以在没有生成代码的情况下执行
