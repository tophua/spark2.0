...
class StreamingQueryListenerSuite extends StreamTest with BeforeAndAfter {
  ...
  testQuietly("single listener, check trigger events are generated correctly") {
    ...
    try {
      // No events until started 没有事件，直到开始
  ...
  //所有的监听器都应该接收QueryTerminatedEvent
  ...
  //没有数据时每个间隔只有一个进度事件
  ignore("only one progress event per interval when no data") {
    // This test will start a query but not push any data, and then check if we push too many events
    //这个测试将启动一个查询，但不会推送任何数据，然后检查是否推送了太多的事件
  ...
  //侦听器只会发布相关会话中启动的查询事件
  ignore("listener only posts events from queries started in the related sessions") {
  ...
  //重放ListenerBus应忽略在2.0.0中生成的破碎事件jsons
  ...
  /** Collects events from the StreamingQueryListener for testing
    * 从StreamingQueryListener收集事件进行测试*/
