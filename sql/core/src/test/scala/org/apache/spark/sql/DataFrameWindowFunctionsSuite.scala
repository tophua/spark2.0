...
class DataFrameWindowFunctionsSuite extends QueryTest with SharedSQLContext {
  ...
  //lead就是取当前顺序的下一条记录,相对Lag()就是取当前顺序的上一行记录
  test("lead") {
    val df = Seq((1, "1"), (2, "2"), (1, "1"), (2, "2")).toDF("key", "value")
...
//lead就是取当前顺序的下一条记录,相对Lag()就是取当前顺序的上一行记录
  ...
  test("rank functions in unspecific window") {
    ...
    df.select(
      ...
      percent_rank().over(Window.partitionBy("value").orderBy("key"))).show()
    checkAnswer(
      df.select(
        $"key",
        max("key").over(Window.partitionBy("value").orderBy("key")),
  ...
  //ExtractWindowExpressions将别名视为常规属性
