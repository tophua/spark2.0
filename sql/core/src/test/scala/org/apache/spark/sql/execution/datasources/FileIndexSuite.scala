...
class FileIndexSuite extends SharedSQLContext {
  ...
  //在PARALLEL_PARTITION_DISCOVERY_THRESHOLD是非正值的情况下，在带有空rootPath的内存文件索引中
  ...
  test("SPARK-20280 - FileStatusCache with a partition with very many files") {
    ...
    * 假的大小，否则我们需要分配2GB的数据来触发这个bug*/
     ...
     * 文件* MyFileStatus.estimatedSize应该溢出到负整数,所以,使其在2bn和4bn之间
     */
