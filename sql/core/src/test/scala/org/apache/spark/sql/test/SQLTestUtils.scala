...
private[sql] trait SQLTestUtils
   ...
   * a file/directory is created there by `f`, it will be delete after `f` returns.
    * 生成一个临时路径而不创建实际的文件/目录,然后将其传递给。如果一个文件/目录是在那里创建的,它将被删除
   *
   ...
   * path of jar's resource which starts with 'jar:file:/'
    *复制文件在坛子的资源到一个临时文件，然后把它传给` F `。
    *这个函数是用来做` F `可以使用临时文件的路径（例如文件：/），而不是对坛子的资源开始的JAR路径：文件：/”
   */
   ...
   * deleted after `f` returns.
    * 创建指定数量的临时目录,然后将其传递给“O”,并在“返回”后删除
   */
  ...
  protected def withUserDefinedFunction(functions: (String, Boolean)*)(f: => Unit): Unit = {
    try {
      ...
      //如果测试失败了一部分,我们不想掩盖失败,因为不能删除没有创建的临时表
   ...
   * 创建一个临时数据库并在执行之前将当前数据库切换到。此数据库在“返回”后被删除。
   ...
   * after `f` returns.
    * 启用区域语言' '执行'之前,然后切换回默认的JVM后，返回'的局域网。
   */
   ...
   * way to construct `DataFrame` directly out of local data without relying on implicits.
    * Turn a logical plan into a `DataFrame`. 这应该被删除后,
    * 我们有一个更简单的方法来构建DataFrame直接从本地数据不依赖于隐式转换
   */
   ...
   * Disable stdout and stderr when running the test. To not output the logs to the console,
    * 禁用stdout和stderr运行测试时,不将日志输出到控制台,
   ...
   * we change System.out and System.err.
    * ConsoleAppender的`遵循`应设置`真`使其荣誉的重新分配的System.out或System.err,否则,
    * consoleappender仍将输出到控制台即使我们改变System.out和System.err。
   */
   ...
   * Run a test on a separate `UninterruptibleThread`.
    * 测试运行在一个单独的` uninterruptiblethread `
   */
   ...
   * FileSystem is changed.
    * 此方法用于使给定路径合格,当路径不包含一个方案时,此路径不会在默认文件系统更改后更改
   */
