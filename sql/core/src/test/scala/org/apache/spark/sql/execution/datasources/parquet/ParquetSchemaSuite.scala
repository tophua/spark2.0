...
class ParquetSchemaSuite extends ParquetSchemaTest {
  test("DataType string parser compatibility") {
    ...
    //这是从以前版本的Spark SQL生成的字符串,使用以下命令：
  ...
  // 测试将Parquet LIST转换为Catalyst ArrayType
  // =======================================================
  //向后兼容性：可空元素类型的LIST - 1 - 标准
  testParquetToCatalyst(
  ...
  testParquetToCatalyst(
    ...
    int96AsTimestamp = true,
    writeLegacyParquetFormat = true)
  //向后兼容性：列表与不可空元素类型 - 5 - parquet-avro
  testParquetToCatalyst(
  ...
  // 测试将Catalyst ArrayType转换为Parquet LIST
  // =======================================================
  ...
  testCatalystToParquet(
    ...
    int96AsTimestamp = true,
    writeLegacyParquetFormat = false)
  //向后兼容性：LIST具有不可为空的元素类型 - 2 - 在1.4.x之前
  testCatalystToParquet(
