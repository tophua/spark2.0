...
/** Dummy provider with only RelationProvider and CreatableRelationProvider.
  * 虚拟提供者只有relationprovider和CreatableRelationProvider*/
...
class DataFrameReaderWriterSuite extends QueryTest with SharedSQLContext with BeforeAndAfter {
  ...
  //没有延伸schemarelationprovider解决默认源
...
//加载测试路径选项
  ...
  //读一个数据源，不延伸schemarelationprovider
  ...
  test("parquet - API and behavior regarding schema") {
    ...
    // Reader, with user specified schema, data should be nulls as schema in file different
    // from user schema
    //读、用户指定的架构,数据应该是空值作为文件不同于用户模式架构
    val expData = Seq[String](null, null, null)
  ...
  //saveastable模式追加不应该不存在但同名临时视图表不存在
  ...
  //saveastable模式追加不应该失败，如果表中已经存在同名的视图和温度存在
  ...
  //模式errorifexists saveastable不应该不存在但同名临时视图表不存在
  ...
  //模式覆盖saveastable不应临时视图不存在但名称相同温度存在的表观
  ...
  //模式覆盖saveastable不应该失败，如果表中已经存在同名的视图和温度存在
  ...
  //saveastable模式忽视应该不存在但同名临时观存在的表创建表
  ...
  //追加一个时段表使用dataframewriter不匹配的对敲
