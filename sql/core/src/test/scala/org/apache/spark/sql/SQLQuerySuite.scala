...
class SQLQuerySuite extends QueryTest with SharedSQLContext {
  ...
  test("SPARK-6743: no columns from cache") {
    ...
    //sql("SELECT t1.b FROM cachedData, cachedData t1 GROUP BY t1.b").show()
  ...
  test("SPARK-7158 collect and take return different results") {
    ...
    // will be different
    //由于ID只是物化一次,那么所有的记录应该来自缓存,而不是通过重新计算,否则,ID将会不同
  ...
  //SET mapreduce.job.reduces自动转换为spark.sql.shuffle.partitions
  ...
  //如果按缺少属性排序,则SELECT distinct不能正常工作
  ...
  //使用UnsafeRows时,SortMergeJoin返回错误的结果
  ...
  test("SPARK-13056: Null in map value causes NPE") {
    ...
    withTempView("maptest") {
      ...
      //局部优化将通过代码生成代码，所以我们应该保持滤波器`关键= 1 `
  ...
  test("check code injection is prevented") {
    ...
    //所以如果评论的结尾由查询中的那些字符组成,我们需要将它们转义
    literal =
      """|\\u002A/
         |{
         |  new Object() {
  ...
  //runtimereplaceable函数不应该采取额外的参数
  ...
  //对inferfiltersfromconstraints和constantpropagation规则冲突
