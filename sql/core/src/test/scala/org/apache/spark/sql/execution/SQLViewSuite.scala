...
//在永久视图上创建一个临时视图
  ...
  //在CREATE VIEW IF NOT EXISTS时存在具有重复名称的表
  ...
  test("error handling: fail if the temp view name contains the database prefix") {
    ...
    //完全合格的表名如“database.table”不允许用于临时视图
    val e = intercept[AnalysisException] {
  ...
  //错误处理：禁止IF NOT EXISTS for CREATE TEMPORARY VIEW
  ...
  //当存在具有相同名称的TEMPORARY VIEW时，不应该允许CREATE TEMPORARY VIEW
  ...
  //当存在具有相同名称的永久VIEW时，应该允许CREATE TEMPORARY VIEW
  ...
  //当存在具有相同名称的TEMPORARY VIEW时，应允许CREATE永久VIEW
  ...
  //正确处理CREATE OR REPLACE TEMPORARY VIEW
  ...
  //如果视图名称没有数据库部分，ALTER VIEW AS应该先尝试修改临时视图
  ...
  //如果视图名称包含数据库部分，ALTER VIEW AS应该改变永久视图
  ...
  //ALTER VIEW AS应该保留以前的表属性，注释，create_time等
  ...
  test("create view for partitioned parquet table") {
    ...
    //分区parquet表不是与hive兼容的,请确保新的标志修复它。
    withTable("parTable") {
      withView("testView") {
  ...
  test("resolve a view when the dataTypes of referenced table columns changed") {
    withTable("tab1") {
      ...
      withView("testView") {
        ...
        //从DoubleType投射到LongType可能会截断，抛出一个AnalysisException。
        ...
        //无法从ArrayType转换为LongType，抛出一个AnalysisException
  ...
  test("correctly handle a cyclic view reference") {
    withView("view1", "view2", "view3") {
      ...
      //检测CREATE OR REPLACE VIEW上的循环视图引用
