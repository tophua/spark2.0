...
private[sql] trait ParquetTest extends SQLTestUtils {
   ...
   * returns.
    * 将`data`写入Parquet文件，然后传递给`f`并在`f`返回后被删除。
   */
   ...
   * Parquet file will be dropped/deleted after `f` returns.
    * 将`data`写入Parquet文件，将其读回为[[DataFrame]]并将其注册为名为`tableName`的临时表,然后调用`f`。
    * 临时表与Parquet文件一起在`f'返回后被删除/删除。
    *
   */
   ...
   * Parquet schema.
    * 这是上面的`writeMetadata`的一个重载版本,允许编写自定义的Parquet模式
   */
  protected def writeMetadata(
