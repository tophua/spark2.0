...
class SessionStateSuite extends SparkFunSuite
   ...
   * with all Hive test suites.
    * 这个套件中的所有测试共享SparkSession,请确保您重置此会话的任何更改,因为这是HiveSessionStateSuite中的单例HiveSparkSession,
    * 并与所有Hive测试套件共享。
   */
  ...
  test("fork new sessions and run query on inherited table") {
    ...
    //由于编译器的限制，不能使用`import activeSession.implicits._`。
