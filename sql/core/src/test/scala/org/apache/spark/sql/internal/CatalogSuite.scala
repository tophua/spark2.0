...
class CatalogSuite
  ...
  //如果同名表存在,dropTempView不应该缓存并删除Metastore表
  ...
  test("createTable with 'path' in options") {
    withTable("t") {
      withTempDir { dir =>
        ...
        // specified.
        //如果指定了自定义表格路径,则表格路径和数据文件在DROP TABLE之后仍然存在
        assert(dir.exists() && dir.listFiles().nonEmpty)
      }
    }
  }
  //选项中没有“路径”的createTable
  test("createTable without 'path' in options") {
    withTable("t") {
      ...
      //如果未指定自定义表格路径，则在DROP TABLE之后删除表格路径
  ...
  test("clone Catalog") {
    ...
    // independence 独立
    dropTable("my_temp_table") // drop table in original session 在原始会话中删除表
